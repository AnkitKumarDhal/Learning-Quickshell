...
Singleton {
    ...
    function scoreText(text, query, exactScore, prefixScore, containsScore, fuzzyBase) {
        ...
        if (acronym >= 0) return acronym
        const fuzzy = root.fuzzyScore(normalized, query)
        if (fuzzy >= 0) return fuzzyBase + fuzzy
    ...
    function scoreApp(app, query, recentKeys, pinnedKeys) {
        ...
        if (score < 0) score = root.scoreText(app.comment, query, 4600, 4350, 4100, 950)
        if (score < 0) score = root.scoreText((app.categories || []).join(" "), query, 3200, 3050, 2900, 550)
        if (score < 0) return -1

        const key = root.appKey(app)

        if (pinnedKeys.indexOf(key) !== -1) score += 100
        const recentIndex = recentKeys.indexOf(key)

        if (recentIndex !== -1) score += Math.max(0, 250 - recentIndex * 10)
        return score
    }
    ...
    function detectMode(query) {
        ...
        if (query.startsWith("?"))                                                       return "web"
        if (/^!g(?:\s|$)/i.test(query))                                                  return "google"
        if (/^!s(?:\s|$)/i.test(query))                                                  return "startpage"
        if (LauncherConvertService.parseUnit(query))                                     return "unit"
        if (/^[-+]?\d+(?:\.\d+)?\s+[a-z]{3}\s+(?:in|to|\/|→)\s+[a-z]{3}$/i.test(query))  return "currency"
        if (/\d/.test(query) && /^[0-9+\-*/%^().,\sA-Za-z]+$/.test(query))               return "calculator"
        if (/^(?:pi|e)$/i.test(query))                                                   return "calculator"
    ...
    function search(apps, query, recentKeys, pinnedKeys) {
        ...
        if (mode === "unit") {
            ...
            return {
                mode: mode,
                results: [],
                title: result !== null && Number.isFinite(result)
                        ? String(Number(result.toFixed(10))) + " " + parsed.to
                        : "Invalid conversion",
                text: result !== null && Number.isFinite(result)
                        ? "Press Enter to copy the result"
                        : "Try 10 km to mi, 100 kmh to mph, or 2 gb to mb",
                detail: parsed
                        ? parsed.value + " " + parsed.from + " → " + parsed.to
                        : "",
        ...
        if (mode === "currency") {
            ...
            if (!LauncherConvertService.currencyLoading &&
                !(
            ...
            return {
                mode: mode,
                results: [],
                title: LauncherConvertService.currencyLoading
                        ? "Converting…"
                        : LauncherConvertService.currencyError
                            ? "Unable to fetch exchange rate"
                            : LauncherConvertService.currencyResult !== ""
                                ? LauncherConvertService.currencyResult + " " + parsed.to
                                : "Fetching exchange rate…",
                text: LauncherConvertService.currencyError
        ...
        for (let i = 0; i < apps.length; i++) {
            const app = apps[i]
            const key = root.appKey(app)
            if (!key || app.noDisplay || seen[key]) continue

            seen[key] = true
            if (normalized === "") {
                results.push({ app: app, score: 0 })
                continue
            }
